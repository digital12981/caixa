...
export class PaymentService {
  ...
  async createPixPayment(data: PaymentRequest): Promise<PaymentResponse> {
    try {
      ...
      // Em desenvolvimento, criar um PIX de demonstração se a API falhar
      if (process.env.NODE_ENV === 'development' || !process.env.NODE_ENV) {
        console.log('Modo desenvolvimento: gerando PIX de demonstração');
        const mockPixCode = '00020126580014br.gov.bcb.pix0136' + 
          Math.random().toString(36).substring(2, 15) + 
          '520400005303986540564.905802BR5913LEILOES CAIXA6009SAO PAULO62290525' +
          Date.now().toString().substring(-8) + '6304' +
          Math.random().toString(36).substring(2, 6).toUpperCase();
          
        return {
          id: `demo_${Date.now()}`,
          pixCode: mockPixCode,
          pixQrCode: `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(mockPixCode)}`,
          status: 'pending'
        };
      }
